                            ...
                            int32_t count)
{
    ...
    for (unsigned attempt = 1; attempt <= 5; ++attempt) {
        ret = i2c_master_transmit(dev_handle, tx_buffer, (size_t)count + 1U, I2C_XFER_TIMEOUT_MS);
        if (ret == ESP_OK) {
    ...
    ESP_LOGE(TAG,
             "I2C write failed reg=0x%02X len=%ld data0=0x%02X: %s",
             index,
             (long)count,
             pdata[0],
             esp_err_to_name(ret));
                           ...
                           int32_t count)
{
    ...
    ESP_LOGE(TAG,
             "I2C read failed reg=0x%02X len=%ld: %s",
             index,
             (long)count,
             esp_err_to_name(ret));
    return STATUS_FAIL;
